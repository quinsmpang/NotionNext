...
import { getOrphanMenuPageCandidates } from '@/lib/site/routeContent'
...
/**
 * 获取优先预生成的页面
 * - 所有独立 Page（包括仅通过链接访问的 Invisible 页面，如 /about）
...
export function getPriorityPages(allPages) {
  const standalonePages = [
    ...
    ...getOrphanMenuPageCandidates(allPages)
  ]
  const publishedPosts = (allPages ?? []).filter(
    page => page.type === 'Post' && page.status === 'Published'
  )

  const top5Default = publishedPosts.slice(0, 5)
  const top5Latest = [...publishedPosts]
    .sort((a, b) => new Date(b.publishDate) - new Date(a.publishDate))
    .slice(0, 5)

  const seen = new Set()
  return [...standalonePages, ...top5Default, ...top5Latest].filter(page => {
