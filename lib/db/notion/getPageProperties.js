 ...
 * 扫描前 50 个一级块，并递归搜索已返回的子块（处理 column / toggle 等容器）
 * @param {string[]} contentIds 页面一级子块 ID 列表
 * @returns {Promise<string>}
 */
async function findFirstBodyImage(contentIds) {
  try {
    const idsToFetch = contentIds.slice(0, 200)
    const recordMap = await notionAPI.getBlocks(idsToFetch)
    const blocks = recordMap?.recordMap?.block || recordMap?.block || {}
    ...
    const extractImageUrl = block => {
      ...
      for (const source of sources) {
        if (typeof source === 'string' && source) {
          const url = mapImgUrl(source, block, 'block')
          if (url) return url
        }
      }
      return ''
    ...
    const searchBlock = (blockId, visited = new Set()) => {
      ...
      for (const childId of childIds) {
        const childUrl = searchBlock(childId, visited)
