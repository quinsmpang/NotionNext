...
export default async function getPageProperties(
  ...
  const pageCoverSource = value?.format?.page_cover
  properties.pageCover = mapImgUrl(pageCoverSource, value) ?? ''
  properties.pageCoverThumbnail =
    mapImgUrl(pageCoverSource, value, 'block') ?? ''

  // 未设置封面或仅使用 Notion 内置纯色封面时，列表优先展示正文第一张图片。
  if (
    properties.type === 'Post' &&
    (!properties.pageCoverThumbnail ||
      isNotionSolidColorCover(pageCoverSource)) &&
    value?.content?.length
  ) {
    const bodyImage = await findFirstBodyImage(value.content)
    if (bodyImage) {
      properties.pageCoverThumbnail = bodyImage
    }
    if (properties.pageCoverThumbnail && !properties.pageCover) {
      properties.pageCover = properties.pageCoverThumbnail
    }
  }
...
/**
 * 判断是否为 Notion 内置的纯色页面封面。
 * 这类图片只是颜色占位，不应覆盖正文首图作为列表缩略图。
 * @param {*} cover
 * @returns {boolean}
 */
export function isNotionSolidColorCover(cover) {
  return (
    typeof cover === 'string' &&
    /\/images\/page-cover\/solid_[^/?#]+\.png(?:[?#].*)?$/i.test(cover)
  )
}

/**
 * 兼容 Notion 新旧接口的多层 value 包装，取出真实 block。
 * @param {*} entry
 * @returns {*}
 */
export function unwrapNotionBlock(entry) {
  let current = entry

  for (let depth = 0; depth < 5 && current; depth++) {
    if (current.id || current.type || current.properties) return current
...
/**
 * 从 getBlocks 返回值中取出 block map。
 * @param {*} response
 * @returns {Record<string, *>}
 */
export function getBlockMapFromResponse(response) {
  return (
    response?.recordMap?.block ||
    response?.recordMapWithRoles?.block ||
    response?.block ||
    {}
  )
}
 ...
 * @returns {string}
 */
export function getImageUrlFromBlock(entry) {
  const block = unwrapNotionBlock(entry)
  if (block?.type !== 'image') return ''

  const sources = [
    block?.properties?.source?.[0]?.[0],
    block?.properties?.source?.[0]?.[1]?.[0]?.[0],
  ...
  for (const source of sources) {
    if (typeof source === 'string' && source) {
      return mapImgUrl(source, block, 'block') || ''
    }
  }
...
/**
 * 从页面正文顶部抓取第一张图片，作为列表缩略图兜底。
 * 按正文顺序扫描一级块，并按需补拉 column / toggle 等容器里的子块。
 * @param {string[]} contentIds 页面一级子块 ID 列表
 * @returns {Promise<string>}
 */
export async function findFirstBodyImage(contentIds) {
  try {
    const blocks = {}
    ...
    const fetchMissingBlocks = async ids => {
      const missingIds = ids
        .filter(id => id && !blocks[id] && !visited.has(id))
        .slice(0, MAX_BLOCKS - visited.size)
      if (!missingIds.length) return

      const response = await notionAPI.getBlocks(missingIds)
      Object.assign(blocks, getBlockMapFromResponse(response))
    }

    const searchInOrder = async (ids, depth = 0) => {
      if (!ids?.length || depth > MAX_DEPTH || visited.size >= MAX_BLOCKS) {
        return ''
      }

      await fetchMissingBlocks(ids)

      for (const id of ids) {
        if (visited.has(id)) continue
        visited.add(id)

        const block = unwrapNotionBlock(blocks[id])
        if (!block) continue

        const imageUrl = getImageUrlFromBlock(block)
        if (imageUrl) return imageUrl

        const childUrl = await searchInOrder(block.content, depth + 1)
    ...
    return await searchInOrder(contentIds.slice(0, MAX_BLOCKS))
  } catch (error) {
    console.warn('[findFirstBodyImage] 抓取正文首图失败:', error?.message)
  }
  return ''
}
