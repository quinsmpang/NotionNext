...
export default function SideBar(props) {
  ...
  if (customMenu && siteConfig('CUSTOM_MENU')) {
    ...
    navLinks = [
      ...
      { name: locale.NAV.ARCHIVE, href: '/archive', show: siteConfig('QUINSM_MENU_ARCHIVE', true, CONFIG) },
      { name: locale.COMMON.CATEGORY, href: '/category', show: siteConfig('QUINSM_MENU_CATEGORY', true, CONFIG) },
      { name: locale.COMMON.TAGS, href: '/tag', show: siteConfig('QUINSM_MENU_TAG', true, CONFIG) },
      { name: locale.NAV.SEARCH, href: '/search', show: siteConfig('QUINSM_MENU_SEARCH', true, CONFIG) }
  ...
  return (
    <div className='js-sidebarWrapper'>
      {siteConfig('QUINSM_SIDEBAR_NAV', true, CONFIG) && (
        <aside className='widget widget_text'>
          <h3 className='widget-title'>
            <span className='widget-title-inner'>{locale.NAV.INDEX || '导航'}</span>
          </h3>
      ...
      {siteConfig('QUINSM_SIDEBAR_HOT_POSTS', true, CONFIG) && hotPosts.length > 0 && (
        <aside className='widget widget_hit-posts'>
          <h3 className='widget-title'>
            <span className='widget-title-inner'>{locale.COMMON.LATEST_POSTS || '最新文章'}</span>
          </h3>
      ...
      {siteConfig('QUINSM_SIDEBAR_TAGS', true, CONFIG) && tags.length > 0 && (
        <aside className='widget widget_hot_tags'>
          <h3 className='widget-title'>
            <span className='widget-title-inner'>{locale.COMMON.TAGS || '标签'}</span>
          </h3>
